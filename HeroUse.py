...
max_pvp_level=16                #最大段位，夺王（16）
sum_count=0
def get_winrate(heroid,pvp_level,race_type,data):
    '''通过英雄id，获取该段位、该游戏比赛类型下的胜率'''
    
    use_data=data[(data.hero==heroid) & (data.pvp_level==pvp_level) & (data.subtype==race_type)]
...
def set_times_sum(data):
    ...
    for race_type in race_types:
        ...
        for level in range(1,max_pvp_level+1):
            resoult[race_type][level]=data[(getattr(data,"subtype")==race_type) & (getattr(data,"pvp_level")==level)]['times'].sum()/8
...
def get_userate(heroid,pvp_level,race_type,data,times_sum_dict):
    ...
    try:
        if times_sum_dict[race_type][pvp_level]==0:
            return 0
        return (data[(data.subtype==race_type) & (data.pvp_level==pvp_level)&(data.hero==heroid)]['times'].sum())/times_sum_dict[race_type][pvp_level]
    except ZeroDivisionError:
...
def handle_file(read_file,save_file,th): 
    ...
    for race_type in race_types:
        ...
        for level in range(1,max_pvp_level+1):
            ...
            if level==1:
                to_excel_dict["英雄名称"]={}
                to_excel_dict["英雄品质"]={} 
            to_excel_dict["段位"+str(level)+"使用率"]={}
            to_excel_dict["段位"+str(level)+"胜率"]={}
            ...
            for heroid in hero_ids:
                ...
                to_excel_dict["段位"+str(level)+"使用率"][heroid]=get_userate(heroid,level,race_type,hero_use_data,times_sum_dict)  
                to_excel_dict["段位"+str(level)+"胜率"][heroid]=get_winrate(heroid,level,race_type,hero_use_data)
